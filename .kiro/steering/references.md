...
Primary-source Lumagen docs and third-party drivers live in the **sibling repo** `esphome-lumagen/References/` (gitignored, local-only). They are the canonical reference for everything `aiolumagen` parses or emits.
...
| `radiance_pro120325/` | Reverse-engineered firmware updater + extracted blobs + Python re-implementation. Not currently consumed by `aiolumagen`. |
